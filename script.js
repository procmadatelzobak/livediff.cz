document.addEventListener('DOMContentLoaded', () => {
  ...
  // NEW connections for the full AnKap map
  const nodeConnections = [
    ['node-uvod', 'node-principy'],
    ['node-uvod', 'node-kritika'],
    ['node-uvod', 'node-reseni'],
    ['node-reseni', 'node-obrana'],
    ['node-reseni', 'node-myty'],
    ['node-principy', 'node-obrana']
  ];
  ...
  function initializeMap() {
    ...
    drawConnectionLines();
    if (activeNode) {
      zoomToNode(activeNode); // Re-zoom to active node
    } else {
      zoomOut(); // Re-apply overview
    }
  }
  ...
  // Recalculate overview on window resize
